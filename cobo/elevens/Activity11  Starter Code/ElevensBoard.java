...
public class ElevensBoard extends Board {
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		List<Integer> tripleIndexes = new ArrayList<Integer>();
		int foundJack = -1;
		int foundQueen = -1;
		int foundKing = -1;
		for (Integer kObj : selectedCards) {
		 ...
		 if (foundJack > -1 && foundQueen > -1 && foundKing > -1) {
			 tripleIndexes.add(foundJack); 
			 tripleIndexes.add(foundQueen); 
			 tripleIndexes.add(foundKing); 
	...
	private boolean playPairSum11IfPossible() {
		...
		List<Integer> playable = findPairSum11(pos);
	...
	private boolean playJQKIfPossible() {
		List<Integer> pos = cardIndexes();
		List<Integer> playable = findJQK(pos);
		if (playable.size() == 3) {replaceSelectedCards(playable); return true;}
		return false; // REPLACE !
	}
}
